	...
	*/
	...
	public static function nick($user = 0, $url = 1, $on = 0, $medal = 0)
	{
		...
		*/
		...
		// Вывод ника 
		if ($user == 0)$ank = array('id' => '0', 'nick' => '系统', 'pol' => '1');
		elseif (!$ank)$ank = array('id' => '0', 'nick' => '[删除]', 'pol' => '1');
		...
		*/
		...
		if ($type == 0 || $type == 2)
		{
			...
			else 
			{
				...
				{
					$icons = mysql_fetch_assoc(mysql_query("SELECT * FROM `user_icons` WHERE `id_user` = '$user' AND `time` > '".(time()-604800)."' LIMIT 1"));
					$icon = '<img src="/style/personal_icons/i (' . $icons['id'] . ').png" alt="" class="icon"/> ';
				}
