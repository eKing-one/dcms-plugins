...
echo 'У тебя есть <b>'.$user[balls].'</b> балов сайта!<br/>Защита : '.$q_user[zashita].' <br /> <font color=red>Поменять их на защитупо курсу '.$cena_sila.' баллов = 1 защита </font><br />';
echo '<form action="?action=change" method="post">';
...
if($q_user['zashita']>200){echo 'У вас и так больше 200!<br /> <a href="/vnim.php">Вернуться назад</a>';break;};
if($q_user['zashita_time']>time()){echo 'Время перезарядки не прошло!<br />Она закончиться в '.vremja($q_user[sila_time]).'  <br /><a href="index.php">Вернуться назад</a>';break;};
mysql_query("UPDATE `user` SET `balls` = '".($user['balls']-$numm)."' WHERE `id` = '$user[id]' LIMIT 1");
mysql_query("UPDATE `pit` SET `zashita` = '".($q_user['zashita']+$num)."' WHERE `id_user` = '$user[id]' LIMIT 1");
mysql_query("UPDATE `pit` SET `sila_time` = '".$timer."' WHERE `id_user` = '$user[id]' LIMIT 1");

echo 'Успешно! Шас только через  '.$time_sila.' часа можете купить силу';
break;
};
