...
// GET	
	...
	mysql_query("UPDATE `photo_tender_user` SET `count` = '0' WHERE `id` = '".$id."'");
	mysql_query("DELETE FROM `photo_tender_golos` WHERE `image` = '".$id."'");
	$_SESSION['message'] = 'Голоса фото сброшены';
	header("Location: /photo_tender/admin/null/?id=".$list."");
	exit;	
	}
	...
	{ 	
	$id = abs(intval($_GET['id']));
	mysql_query("UPDATE `photo_tender_user` SET `count` = '0' WHERE `tender` = '".$id."'");
	mysql_query("DELETE FROM `photo_tender_golos` WHERE `tender` = '".$id."'");
	$_SESSION['message'] = 'Все голоса фото сброщены';
	header("Location: /photo_tender/admin/null/?id=".$id."");
	exit;	
	}
...
// заголовок страницы
	...
	$set['title'] = 'Фотоконкурсы : '.$name.' : Сброс голосов фото'; 
...
// style
	...
	<div class="fot_k_3">
	<span class="fot_k_5">
	<a href="/photo_tender/admin/null/?id='.$tender['id'].'&list='.$act['id'].'&type=ok">
	<img src="/photo_tender/ico/top.png" alt="*"></a>
	</span>
	...
	if ($k_page > 1)str("/photo_tender/admin/null/?id=".$act['id']."&", $k_page, $page); // Вывод страниц	
	...
	'.($count > 0 ? '
	<div class="block_y">
	<img src="/photo_tender/ico/top.png" alt="*"> <a href="/photo_tender/admin/null/?id='.$act['id'].'&type=all"> Сбросить все голоса</a>
	</div>':'').'
	<div class="block_y">
