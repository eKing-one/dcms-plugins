...
// GET	
	...
	mysql_query("UPDATE `photo_tender_user` SET `like` = '0' WHERE `id` = '".$id."'");
	mysql_query("DELETE FROM `photo_tender_like` WHERE `image` = '".$id."'");
	$_SESSION['message'] = 'Лайки фото сброшены';
	header("Location: /photo_tender/admin/like/?id=".$list."");
	exit;	
	}
	...
	{ 	
	$id = abs(intval($_GET['id']));
	mysql_query("UPDATE `photo_tender_user` SET `like` = '0' WHERE `tender` = '".$id."'");
	mysql_query("DELETE FROM `photo_tender_like` WHERE `tender` = '".$id."'");
	$_SESSION['message'] = 'Все лайки фото сброщены';
	header("Location: /photo_tender/admin/like/?id=".$id."");
	exit;	
	}
...
// заголовок страницы
	...
	$set['title'] = 'Фотоконкурсы : '.$name.' : Сброс лайков фото'; 
...
// style
	...
	<div class="fot_k_3">
	<span class="fot_k_5">
	<a href="/photo_tender/admin/like/?id='.$tender['id'].'&list='.$act['id'].'&type=ok">
	<img src="/photo_tender/ico/like.gif" alt="*"></a>
	</span>
	...
	if ($k_page > 1)str("/photo_tender/admin/like/?id=".$act['id']."&", $k_page, $page); // Вывод страниц	



	echo'
	</div>
	'.($count > 0 ? '<div class="block_y">
	<img src="/photo_tender/ico/like.gif" alt="*"> <a href="/photo_tender/admin/like/?id='.$act['id'].'&type=all"> Сбросить все лайки</a>
	</div>
	':'').'
	<div class="block_y">
