...
// GET	
	...
	mysql_query("UPDATE `photo_tender_user` SET `closed` = '1' WHERE `id` = '".$id."'");
	$_SESSION['message'] = 'Фото заблокировано';
	header("Location: /photo_tender/admin/block/?id=".$list."");
	exit;	
	}
	if (isset($_GET['type']) && $_GET['type'] == 'no')
	{ 	
	$id = abs(intval($_GET['id']));
	$list = abs(intval($_GET['list']));
	mysql_query("UPDATE `photo_tender_user` SET `closed` = '0' WHERE `id` = '".$id."'");
	$_SESSION['message'] = 'Фото разблокировано';
	header("Location: /photo_tender/admin/block/?id=".$list."");
	exit;	
	}
	...
	{ 	
	$id = abs(intval($_GET['id']));
	mysql_query("UPDATE `photo_tender_user` SET `closed` = '1' WHERE `tender` = '".$id."'");
	$_SESSION['message'] = 'Все фото заблокированы';
	header("Location: /photo_tender/admin/block/?id=".$id."");
	exit;	
	}
	if (isset($_GET['type']) && $_GET['type'] == 'all_no')
	{ 	
	$id = abs(intval($_GET['id']));
	mysql_query("UPDATE `photo_tender_user` SET `closed` = '0' WHERE `tender` = '".$id."'");
	$_SESSION['message'] = 'Все фото разблокированы';
	header("Location: /photo_tender/admin/block/?id=".$id."");
	exit;	
	}
...
// заголовок страницы
	...
	$set['title'] = 'Фотоконкурсы : '.$name.' : Блокировка фото'; 
...
// style
	...
	<div class="fot_k_3">
	<span class="fot_k_5">
	<a href="/photo_tender/admin/block/?id='.$tender['id'].'&list='.$act['id'].'&type='.($tender['closed'] == 1 ? 'no':'ok').'">
	<img src="/photo_tender/ico/info'.($tender['closed'] == 0 ? 'r':'').'.png" alt="*"></a>
	</span>
	...
	if ($k_page > 1)str("/photo_tender/admin/block/?id=".$act['id']."&", $k_page, $page); // Вывод страниц	
	...
	'.($count > 0 ? '
	<div class="block_y">
	<img src="/photo_tender/ico/info.png" alt="*"> <a href="/photo_tender/admin/block/?id='.$act['id'].'&type=all_ok"> Заблокировать все</a>
	</div>
	<div class="block_y">
	<img src="/photo_tender/ico/infor.png" alt="*"> <a href="/photo_tender/admin/block/?id='.$act['id'].'&type=all_no"> Разблокировать все</a>
	</div>
	':'').'
	<div class="block_y">
