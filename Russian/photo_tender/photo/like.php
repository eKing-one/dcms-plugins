...
// ID
	...
	$like = mysql_result(mysql_query("SELECT COUNT(*) FROM `photo_tender_like` WHERE `image` = '$act[id]' AND `user` = '$user[id]' LIMIT 1"), 0);
...
// если все нормаьно
	...
	mysql_query("INSERT INTO `photo_tender_like` (user, image, tender, time) values('$user[id]', '".$act['id']."',".$act['tender'].", '" .$time. "' )");
	mysql_query("UPDATE `photo_tender_user` SET `like` = '".($act['like'] + 1)."' WHERE `id` = '".$act['id']."' LIMIT 1");
...
// отправляем сообщение
	...
	$msg = 'Пользователь '.$user['nick'].' поставил лайк на ваше фото в фотоконкурсе [url=/photo_tender/view/?id='.$tender['id'].'][blue]'.$name.'[/blue][/url].';
	...
	$_SESSION['message'] = 'Лайк принят.';
	header("location: /photo_tender/photo/list.php?id=".$tender['id']."?");
	
	}else{ $_SESSION['message'] = 'Вы уже поставили лайк на фото.'; header("location: /photo_tender/photo/list.php?id=".$tender['id']."?");}
	}else{ $_SESSION['message'] = 'Вы не можете ставить лайк себе.'; header("location: /photo_tender/photo/list.php?id=".$tender['id']."?");}
