...
{
$set['title']=$gruppy['name'].' - Участники'; // заголовок страницы
include_once '../sys/inc/thead.php';
title();
...
echo'<center><b>Участники Группы</b></center>';
...
$q=mysql_query("SELECT * FROM `gruppy_users` WHERE `id_gruppy`='$gruppy[id]' AND `activate`='0' AND `invit`='0' AND `mod`='0' ORDER BY `time` DESC LIMIT $start, $set[p_str]");
...
echo "<div class='nav2'>\n";
$num=1;}
echo ''.online($us['id']).' <a href="info.php?s='.$gruppy['id'].'&id='.$us['id'].'"><span style="color:'.$us['ncolor'].'">'.$us['nick'].'</span></a> ';
if ($us['status_vip']==1)echo "<img src='/status_vip/img/vip.png'></img>\n";
