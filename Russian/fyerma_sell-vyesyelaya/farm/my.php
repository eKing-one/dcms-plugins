...
if (isset($_GET['add_ok'])) add_farm_event('苗床采购成功');
...
if (isset($_GET['watok'])) {
    add_farm_event('你成功地给床浇水了。收获前的时间缩短了 [B] 半小时 [/B]。经验 +1，健康-1。');
}
...
if (isset($_GET['add_no'])) {
    add_farm_event('你的水平太低了，或者你没有足够的钱承诺新的床');
}
...
if (isset($_GET['add'])) {
    ...
    if ($level >= $lvl) {
        echo "<div class='rowup'><img src='/farm/img/garden.png' alt='' /> <a href='/farm/garden/'>你的菜园</a> / <b>添加床</b></div>";
        echo "<div class='rowdown'>一个床的成本为： " . $cost . " <br />";
        ...
        echo "<div class='rowup'><img src='/farm/img/garden.png' alt='' /> <a href='/farm/garden/'>你的菜园</a> / <b>添加床</b></div>";
        echo "<b>你不能加床。</b><br />";
        echo "BAS 缺乏<b>知识</b>。 要添加另一个苗床，你需要更多的<b>经验</b>。<br />";
        echo "没钱？<b>赚钱</b>-在论坛上交流，在日记中写笔记和评论别人的东西，在报纸上写文章，在聊天中交流和猜测问题。<b>方法十！</b><br />";
    }
} else {
    ...
    echo "<div class='mdlc'><span>你的床清单[" . $k_post . "]</span><br /></div>";
    echo "<div class='menu'>";
    ...
    while ($post = dbarray($res)) {
        ...
        if ($post['semen'] == 0) {
            ...
            if ($plcnt != 0) {
                ...
                $act = "<img src='/farm/img/plant.png' alt='' class='rpg' /> Посадить <a href='/farm/gr.php?id=" . $post['id'] . "&plantid=" . $plant['id'] . "&posadka'>" . $plnew['name'] . "</a> или <a href='/farm/gr.php?id=" . $post['id'] . "'>选择不同的植物</a>";
            }
        }
        ...
        if (
            ...
            $secondfield = floor(($timediff - $dayfield * $oneDay - $hourfield * $oneHour - $minutefield * $oneMinute));
            if ($dayfield > 0) $day = $dayfield . '天';
            $time_wost = $day . $hourfield . "是" . $minutefield . "分";
        ...
        if ($post['semen'] != 0 && $post['udobr'] == 1 && $post['time_water'] > time() && $post['time'] > time() && $post['time'] < $post['time_water']) {
            $act = "<img src='/farm/img/time.png' alt='' class='rpg' /> 在收集之前还剩下： " . $time_1 . "";
        }
...
if ($irnum != 0 && $fuser['k_poliv'] != 0) {
    ...
    echo "<div class='rowdown'>";
    echo "<img src='/farm/img/irrigation_small.png' alt='' class='rpg' /> <a href='/farm/shop_combine.php?irrigation'>喷洒器</a> (" . $fuser['k_poliv'] . "/3 Yp., " . $irtime . " 秒., +<img src='/farm/img/exp.png' alt='' />" . $irexp . ")<br />";
    echo "<img src='/farm/img/water.png' alt='' class='rpg' /> <a href='/farm/combine/irrigation.php?start'>给花床浇水</a>";
    echo "</div>";
}
...
if ($plcont != 0) {
    ...
    $pln = dbarray(dbquery("SELECT * FROM `farm_plant` WHERE `id` = '" . $plant['semen'] . "' LIMIT 1"));
    $act = "<img src='/farm/img/plant.png' alt='' class='rpg' /> 种植 <a href='/farm/gr.php?id=" . $post['id'] . "&plantid=" . $plant['id'] . "&posadka'>" . $plnew['name'] . "</a> или <a href='/farm/gr.php?id=" . $post['id'] . "'>选择不同的植物</a>";
}
...
if ($snum != 0 && $fuser['k_posadka'] != 0) {
    ...
    echo "<div class='rowdown'>";
    if ($plcont != 0) {
        echo "<img src='/farm/img/seeder_small.png' alt='' class='rpg' />  <a href='/farm/shop_combine.php?seeder'>播种机</a> (" . $fuser['k_posadka'] . "/3 yp., " . $stime . " сек., +<img src='/farm/img/exp.png' alt='' />" . $sexp . ")<br />";
        ...
        echo "<img src='/farm/img/seeder_small.png' alt='' class='rpg' /> <a href='/farm/combine/seeder.php?select'>用播种机播种：：无种子</a> (" . $fuser['k_posadka'] . "/3 ур.)";
    }
    echo "</div>";
}
...
echo "<img src='/farm/img/village.png' alt='' class='rpg' /> <a href='/farm/stat.php'>统计</a><br/>";
echo "<img src='/farm/img/harvest.png' alt='' class='rpg' /> <a href='/farm/fermers/'>全体农民</a><br/>";

if (!isset($_GET['add'])) {
    echo "<img src='/img/add.png' alt='Купить грядку' class='rpg' /> <a href='/farm/garden/?add'>加床</a> <a href='/farm/help.php'>[?]</a><br/>";
}

echo "<img src='/farm/img/garden.png' alt='Назад' class='rpg' /> <a href='/farm/garden/'>后退</a><br/>";
echo "<img src='/img/back.png' alt='Назад' class='rpg' /> <a href='/farm/'>主</a>";
echo "</div>";
