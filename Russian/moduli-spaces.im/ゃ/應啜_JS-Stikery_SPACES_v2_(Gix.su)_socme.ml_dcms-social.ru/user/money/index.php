...
echo "</div>\n";
echo '<div class="nav2">';$ic = mysql_result(mysql_query("SELECT COUNT(*) FROM `stickers` WHERE `id_user` = '$user[id]' AND `time` > '$time'"), 0);echo '&rarr; <a href="/user/money/pay.php">Стикеры</a> ' . ($ic == 0 ? '<span class="off">[отключена]</span> ' : '<span class="on">[включена]</span>');echo '</div>';
