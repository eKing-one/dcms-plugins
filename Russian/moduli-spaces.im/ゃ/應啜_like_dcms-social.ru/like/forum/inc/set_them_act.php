...
##удаляем рейтинг постов темы##
mysql_query("DELETE FROM `forum_like` WHERE `id_them` = '$them[id]'");
mysql_query("DELETE FROM `forum_like_users` WHERE `id_them` = '$them[id]'");
...
else $add_q=NULL;if (isset($_POST['close']) && $_POST['close']==1 && $them['close']==0){
$close=1;
...
if (!isset($err)){
mysql_query("UPDATE `forum_t` SET `name` = '$name', `text` = '$msg', `id_user` = '$autor',$add_q `close` = '$close' WHERE `id` = '$them[id]' LIMIT 1");
