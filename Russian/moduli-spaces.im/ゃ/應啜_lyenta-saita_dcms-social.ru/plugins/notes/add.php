...
mysql_query("INSERT INTO `notes` (`time`, `msg`, `name`, `id_user`, `private`, `private_komm`, `id_dir`, `type`) values('$time', '$msg', '$title', '$user[id]', '$privat', '$privat_komm', '$id_dir', '$type')");// ---------------- Запись в ленту сайта ----------------- //$set_lenta = mysql_fetch_assoc(mysql_query("SELECT `note` FROM `lenta_site_set`"));$new_note = mysql_fetch_assoc(mysql_query("SELECT `id` FROM `notes` WHERE `id_user` = '$user[id]' ORDER BY `id` DESC"));if($set_lenta['note'] == 1) mysql_query("INSERT INTO `lenta_site` SET `id_user` = '$user[id]', `opis` = ' написал новый дневник ', `title` = '$title', `link` = 'plugins/notes/list.php?id=".$new_note['id']."', `time` = '$time'");// ------------------------ Конец ----------------------- //
$st = mysql_insert_id();
/*
===================================
Лента
...
{
echo "<option value='$post[id]'".($id_dir == $post['id']?" selected='selected'" : null).">" . htmlspecialchars($post['name']) . "</option>\n";
}
echo "</select><br />\n";
