...
$set_lenta = mysql_fetch_assoc(mysql_query("SELECT `note_komm` FROM `lenta_site_set`"));
if($set_lenta['note_komm'] == 1) mysql_query("INSERT INTO `lenta_site` SET `id_user` = '$user[id]', `opis` = ' оставил комментарий в дневнике ', `title` = '".htmlspecialchars($notes['name'])."', `link` = 'plugins/notes/list.php?id=".$notes['id']."', `time` = '$time'");
// ------------------------ Конец ----------------------- //
...
if (isset($user))
$frend=mysql_result(mysql_query("SELECT COUNT(*) FROM `frends` WHERE (`user` = '$user[id]' AND `frend` = '$avtor[id]') OR (`user` = '$avtor[id]' AND `frend` = '$user[id]') LIMIT 1"),0);title();
...
echo '<div class="mess">' . output_text($stat_1) , '</div>'; // вывод статьи со всем форматированием
...
echo "</div>";if (isset($user) && $user['id']!=$avtor['id']){
echo "<div class='main'>";
if (mysql_result(mysql_query("SELECT COUNT(*) FROM `notes_like` WHERE `id_user` = '".$user['id']."' AND `id_notes` = '".$notes['id']."' LIMIT 1"),0)==0)
...
echo "".medal($ank['id'])." ".online($ank['id'])." (".vremja($post['time']).")<br />";$postBan = mysql_result(mysql_query("SELECT COUNT(*) FROM `ban` WHERE (`razdel` = 'all' OR `razdel` = 'notes') AND `post` = '1' AND `id_user` = '$ank[id]' AND (`time` > '$time' OR `navsegda` = '1')"), 0);
