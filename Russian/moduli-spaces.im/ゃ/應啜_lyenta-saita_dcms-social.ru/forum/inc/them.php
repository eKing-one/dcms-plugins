...
$set_lenta = mysql_fetch_assoc(mysql_query("SELECT `forum_komm` FROM `lenta_site_set`"));
if($set_lenta['forum_komm'] == 1) mysql_query("INSERT INTO `lenta_site` SET `id_user` = '$user[id]', `opis` = ' оставил комментарий в теме форума ', `title` = '".htmlspecialchars($them['name'])."', `link` = 'forum/".$forum['id']."/".$razdel['id']."/".$them['id']."/', `time` = '$time'");
// ------------------------ Конец ----------------------- //
