...
$set_lenta = mysql_fetch_assoc(mysql_query("SELECT `reg` FROM `lenta_site_set`"));
$new_us = mysql_fetch_assoc(mysql_query("SELECT `id` FROM `user` ORDER BY `id` DESC"));
if($set_lenta['reg'] == 1) mysql_query("INSERT INTO `lenta_site` SET `id_user` = '1', `opis` = ' На нашем сайте новенький! Приветствуем ', `title` = '$_SESSION[reg_nick]', `link` = 'info.php?id=".$new_us[id]."', `time` = '$time'");
// ------------------------ Конец ----------------------- //
...
{if ($webbrowser == 'web'){	echo '<div class="mess">Войти через:<br /><script src="//ulogin.ru/js/ulogin.js"></script><div id="uLogin" data-ulogin="display=panel;fields=first_name,last_name,city,sex,photo,photo_big;providers=vkontakte,odnoklassniki,mailru,facebook,yandex;hidden=;redirect_uri=http%3A%2F%2F'.$_SERVER['HTTP_HOST'] . '/user/connect/loginAPI.php"></div></div>';}else{	?><div class='mess'>Войти через:<br /><script src="//ulogin.ru/js/ulogin.js"></script><div id="uLogin" data-ulogin="display=small;fields=first_name,last_name,city,sex,photo,photo_big;providers=vkontakte,odnoklassniki,mailru,facebook,yandex;hidden=;redirect_uri=http%3A%2F%2F<?echo $_SERVER['HTTP_HOST'] . '/user/connect/loginAPI.php';?>"></div></div><?}
