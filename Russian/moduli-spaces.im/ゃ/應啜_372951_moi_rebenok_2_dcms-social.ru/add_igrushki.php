...
if (!isset($err))
{
mysql_query("INSERT INTO `baby_shop_igrushki` (`name`, `cena`) VALUES ('".$name."', '".$cena."')");
$ii=mysql_insert_id();
if (preg_match('#\.jpe?g$#i',$_FILES['foto']['name']) || preg_match('#\.gif$#i',$_FILES['foto']['name']) || preg_match('#\.png$#i',$_FILES['foto']['name']))
{
copy($_FILES['foto']['tmp_name'], H."baby/shop_igrushki/".$ii.".png");
@chmod(H."baby/shop_igrushki/".$ii.".png" , 0777);
}
echo "<div class='msg'>";
echo "Игрушка успешно добавлена!";
echo "</div>";
}
}
err();
