...
if (isset($_GET['url']))$return = urldecode($_GET['url']);
else $return = NULL;
...
echo '
     ...
     <form method="post" action="?meta_title=' . urlencode($Mtitle) . '&amp;url=' . urlencode($return) . '">

     <b>TITLE страницы(не нужно изменять если указан):</b><br />
     <input name="meta_title" value="' . (!empty($Mall['title']) ? htmlentities($Mall['title'], ENT_QUOTES, 'UTF-8') : NULL) . '" type="text" /><br />

     <b>Ключевые слова (META KEYWORDS):</b><br />
     <textarea name="meta_keywords">' . (!empty($Mall['meta_keywords']) ? htmlentities($Mall['meta_keywords'], ENT_QUOTES, 'UTF-8') : NULL) . '</textarea><br />

     <b>Описание (META DESCRIPTION):</b><br />
<textarea name="meta_description">' . (!empty($mall['meta_description']) ? htmlentities($Mall['meta_description'], ENT_QUOTES, 'UTF-8') : NULL) . '</textarea><br />
