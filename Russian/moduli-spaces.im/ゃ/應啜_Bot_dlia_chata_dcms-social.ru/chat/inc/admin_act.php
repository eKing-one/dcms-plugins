...
if ($_POST['bots']==4)$bot=1;else $bot=0;
mysql_query("UPDATE `chat_rooms` SET `name` = '$name', `opis` = '$opis', `pos` = '$pos', `umnik` = '$umnik', `shutnik` = '$shutnik', `bot` = '$bot' WHERE `id` = '$room[id]' LIMIT 1");
...
if ($_POST['bots']==2 || $_POST['bots']==3)$shutnik=1;else $shutnik=0;
if ($_POST['bots']==4)$bot=1;else $bot=0;
mysql_query("INSERT INTO `chat_rooms` (`name`, `opis`, `pos`, `umnik`, `shutnik`, `bot`) values('$name', '$opis', '$pos', '$umnik', '$shutnik', '$bot')");
