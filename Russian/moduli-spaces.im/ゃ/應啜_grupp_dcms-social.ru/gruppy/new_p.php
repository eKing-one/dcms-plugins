...
$q=mysql_query("SELECT * FROM `gruppy_forum_thems` WHERE `id_gruppy`='$s' ORDER BY `time` DESC LIMIT $start, $set[p_str]");
...
$forum=mysql_fetch_assoc(mysql_query("SELECT * FROM `gruppy_forums` WHERE `id` = '$them[id_forum]' LIMIT 1"));
...
$post1=mysql_fetch_assoc(mysql_query("SELECT * FROM `gruppy_forum_mess` WHERE `id_them` = '$them[id]' AND `id_gruppy` = '$s' AND `id_forum` = '$forum[id]' ORDER BY `time` ASC LIMIT 1"));
...
$post=mysql_fetch_assoc(mysql_query("SELECT * FROM `gruppy_forum_mess` WHERE `id_them` = '$them[id]' AND `id_gruppy` = '$s' AND `id_forum` = '$forum[id]' ORDER BY `time` DESC LIMIT 1"));
...
echo "<div class=\"foot\">\n";
echo "&raquo;<a href=\"new_t.php?s=$gruppy[id]\">Новые темы</a><br />\n";
echo "</div>\n";
echo "<div class='foot'>\n";
echo "<img src='img/back.png' alt='' class='icon'/> <a href=\"index.php?s=$s\" title=\"Вернуться в сообщество\">В сообщество</a><br />\n";
echo "</div>\n";
}
else
{
