...
if (isset($_GET['id']) && isset($_GET['black']) &&  isset($user) && $user['id'] != $ank['id'] && mysql_result(mysql_query("SELECT COUNT(*) FROM `black_list` WHERE `id_user` = '".$user['id']."' AND `id_black_user` = '".$ank['id']."' "), 0) == 0 && $ank['level']==0) {

mysql_query("INSERT INTO `black_list` (  `id_user`, `id_black_user`, `time`) values('".$user['id']."', '".$ank['id']."', '$time') ");

$_SESSION['message'] = 'Пользователь добавлен в ваш черный список';

}
if (isset($_GET['id']) && isset($_GET['out_black']) &&  isset($user) && $user['id'] != $ank['id']) {

mysql_query("DELETE FROM `black_list` WHERE `id_user` = '".$user['id']."' AND `id_black_user` = '".$ank['id']."' LIMIT 1");

$_SESSION['message'] = 'Пользователь удален из черного списка';
}
...
mysql_query("INSERT INTO `my_guests` (`id_ank`, `id_user`, `time`) VALUES ('$ank[id]', '$user[id]', '$time')");mysql_query("UPDATE `user` SET `rating_tmp` = '".($ank['rating_tmp']+1)."' WHERE `id` = '$ank[id]' LIMIT 1");
...
mysql_query("UPDATE `notification` SET `read` = '1' WHERE `type` = 'stena_komm' AND `id_user` = '$user[id]' AND `id_object` = '$ank[id]'");
...
mysql_query("INSERT INTO `status` (`id_user`, `time`, `msg`, `pokaz`) values('$user[id]', '$time', '".my_esc($msg)."', '1')");
...
if (isset($_GET['fav']) && isset($user)){
	...
	if (mysql_result(mysql_query("SELECT COUNT(*) FROM `mark_people` WHERE `id_user` = '".$user['id']."' AND `id_people` = '".$ank['id']."' LIMIT 1"),0) == 0 && $_GET['fav'] == 1){

		mysql_query("INSERT INTO `mark_people` (`id_people`, `id_user`, `time`) VALUES ('$ank[id]', '$user[id]', '$time')");
	...
	if (mysql_result(mysql_query("SELECT COUNT(*) FROM `mark_people` WHERE `id_user` = '".$user['id']."' AND `id_people` = '".$ank['id']."' LIMIT 1"),0) == 1 && $_GET['fav'] == 0){

		mysql_query("DELETE FROM `mark_people` WHERE `id_user` = '$user[id]' AND  `id_people` = '$ank[id]'");
