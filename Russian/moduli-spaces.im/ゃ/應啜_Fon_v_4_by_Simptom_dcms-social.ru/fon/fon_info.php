...
Error_Reporting(0);
only_reg();
$set['title']='Выгрузка фона на страничку';
include_once '../sys/inc/thead.php';
title();
if (isset($_GET['act']) && $_GET['act']=='delete' && isset($_GET['ok']))
{
@unlink(H."fon/img_info/$user[id].jpg");
@unlink(H."fon/img_info/$user[id].gif");
@unlink(H."fon/img_info/$user[id].png");
...
imagedestroy($imgc);
@chmod(H."fon/img_info/$user[id].jpg",0777);
@chmod(H."fon/img_info/$user[id].gif",0777);
@chmod(H."fon/img_info/$user[id].png",0777);
@unlink(H."fon/img_info/$user[id].jpg");
@unlink(H."fon/img_info/$user[id].gif");
@unlink(H."fon/img_info/$user[id].png");
imagejpeg($screen,H."fon/img_info/$user[id].jpg",100);
@chmod(H."fon/img_info/$user[id].jpg",0777);
imagedestroy($screen);
}
else
{
copy($_FILES['file']['tmp_name'], H."fon/img_info/$user[id].jpg");
}
...
@unlink(H."fon/img_info/$user[id].gif");
@unlink(H."fon/img_info/$user[id].png");
file_put_contents(H."fon/img_info/$user[id].gif", $screen);
@chmod(H."fon/img_info/$user[id].gif",0777);
msg("Фон успешно загружен!");
$price = $user['balls'] - 150;
mysql_query("UPDATE `user` SET `balls` = '$price' WHERE `id` = '".$user['id']."'");
...
@unlink(H."fon/img_info/$user[id].gif");
@unlink(H."fon/img_info/$user[id].png");
imagepng($screen,H."fon/img_info/$user[id].png");
@chmod(H."fon/img_info/$user[id].png",0777);
imagedestroy($screen);
}
else
{
copy($_FILES['file']['tmp_name'], H."fon/img_info/$user[id].png");
}
msg("Фон успешно загружен!");
$price = $user['balls'] - 150;
mysql_query("UPDATE `user` SET `balls` = '$price' WHERE `id` = '".$user['id']."'");
...
err();
aut();
$fon_info = mysql_fetch_array(mysql_query("SELECT * FROM `fon_info` WHERE `user_id` = '".$user['id']."'"));
...
echo '<a href="/fon/fon_info/index.php?act=ok&amp;select=0">Продать фон</a><br />';
echo '</div>';
}else{
if (is_file(H."fon/img_info/$user[id].gif") || is_file(H."fon/img_info/$user[id].jpg") || is_file(H."fon/img_info/$user[id].png"))
{
