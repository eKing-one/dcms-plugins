...
$set['title']='Фон личной странички';
Error_Reporting(0);
...
echo 'После покупки фона все обитатели, посетившие вашу страничку увидят ее в новом оформлении. Фон на вашей страничке будет в течении недели. После он исчезнет.<br/><hr>';
...
echo '<a href="/fon/fon_info.php?act=delete">Удалить фон</a><br />';
echo '</div>';
}else{
if ($user['fon_info_vibor']==0)
{
$fon_info = mysql_fetch_array(mysql_query("SELECT * FROM `fon_info` WHERE `user_id` = '".$user['id']."'"));
...
echo "<a href='/fon/fon_info/'>Назад</a><br />\n";
echo "</div>\n";
break;
case 'ok':
...
mysql_query("UPDATE `fon_info` SET `fon_id` = '$select', `time` = '$realtime' WHERE `user_id` = '".$user['id']."'");
...
default:
echo 'Хотите что бы ваша страничка отличалась от остальных красивым фоном?<br/>';
...
echo "</div>\n";
if (!isset($fon_info['user_id'])) mysql_query("INSERT INTO `fon_info` SET `user_id` = '".$user['id']."'");
break;
