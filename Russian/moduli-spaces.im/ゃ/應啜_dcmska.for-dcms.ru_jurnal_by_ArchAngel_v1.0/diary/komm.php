...
title();
if (isset($_POST['msg']) && isset($user) && ($us['id']==$user['id'] || $user['level']>$us['level'] || $diary['readers']==0 || ($diary['readers']==1 || $diary['readers']==2) && mysql_result(mysql_query("SELECT COUNT(*) FROM `frends` WHERE (`user` = '$user[id]' AND `frend` = '$us[id]') OR (`user` = '$us[id]' AND `frend` = '$user[id]')"),0)!=0))
{
$msg=esc(stripcslashes(htmlspecialchars($_POST['msg'])));
if (isset($_POST['translit']) && $_POST['translit']==1)$msg=translit($msg);
if (strlen2($msg)>1024){$err='Сообщение слишком длинное';}
elseif (strlen2($msg)<2){$err='Короткое сообщение';}
elseif (mysql_result(mysql_query("SELECT COUNT(*) FROM `diary_komm` WHERE `id_diary` = '$diary[id]' AND `id_user` = '$user[id]' AND `msg` = '".mysql_real_escape_string($msg)."' LIMIT 1"),0)!=0){$err='Ваше сообщение повторяет предыдущее';}
else{
mysql_query("INSERT INTO `diary_komm` (`id_diary`, `id_user`, `time`, `msg`) values('$diary[id]', '$user[id]', '$time', '".my_esc($msg)."')");
mysql_query("UPDATE `user` SET `balls` = '".($user['balls']+1)."' WHERE `id` = '$user[id]' LIMIT 1");
if($user['id']!=$us['id'])
{
if($user['pol']==1)$pol='оставил'; else $pol='оставила';
mysql_query("INSERT INTO `jurnal` (`id_user`, `id_kont`, `msg`, `time`, `type`) values('0', '$us[id]', '[url=/info.php?id=$user[id]]$user[nick][/url] $pol комментарий к дневнику [url=/diary/$diary[name]/]$diary[name][/url]', '$time', 'blog')");}
$q = mysql_query("SELECT * FROM `frends` WHERE `user` = '$user[id]' AND `lenta_blog` = '1' AND `i`='1'");
while ($f = mysql_fetch_array($q))
{
$a = mysql_fetch_array(mysql_query("SELECT * FROM `user` WHERE `id` = '$f[id_friend]' LIMIT 1"));
...
elseif (isset($_GET['del']) && mysql_result(mysql_query("SELECT COUNT(*) FROM `diary_komm` WHERE `id` = '".intval($_GET['del'])."' AND `id_diary` = '$diary[id]'"),0))
{
...
if (isset($user) && ($user['level']>=3 || $user['id'] == $diary['id_user']))
echo'<a href="?id='.$diary['id'].'&del='.$post['id'].'">Удалить</a><br />';
echo'</td>';
echo'</tr>';
}
echo'</table>';
...
if (isset($user) && ($us['id']==$user['id'] || $user['level']>$us['level'] || $diary['readers']==0 || ($diary['readers']==1 || $diary['readers']==2) && mysql_result(mysql_query("SELECT COUNT(*) FROM `frends` WHERE (`user` = '$user[id]' AND `frend` = '$us[id]') OR (`user` = '$us[id]' AND `frend` = '$user[id]')"),0)!=0))
{
echo'<form method="post" name="message" action="?id='.$diary['id'].'">';
if ($set['web'] && is_file(H.'style/themes/'.$set['set_them'].'/altername_post_form.php'))
include_once H.'style/themes/'.$set['set_them'].'/altername_post_form.php';
else
echo'Сообщение|<a href="/smiles/index.php">Смайлы</a>|<a href="http://atrp.net.ru/pages/bb_code.php">BB-Code</a><br /><textarea name="msg"></textarea><br />';
...
echo'<div class="foot">';
echo'<a href="/diary/'.$diary['name'].'/" title="Вернуться в дневник '.$diary['name'].'">Назад</a><br />';
