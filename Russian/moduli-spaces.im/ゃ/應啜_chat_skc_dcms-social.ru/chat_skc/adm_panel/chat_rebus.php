...
$set['title']='Чат - Ребусы';
include_once '../sys/inc/thead.php';
title();
if (isset($_GET['act']) && isset($_POST['sep']) && isset($_FILES['file']['tmp_name']))
{
$sep=$_POST['sep'];
if (isset($_POST['replace']))mysql_query('TRUNCATE `rebus_vopros`');
$k_add=0;
...
mysql_query("INSERT INTO `rebus_vopros` (`vopros`, `otvet`) VALUES ('".my_esc($vopr)."', '".my_esc($otv)."')");
$k_add++;
}
...
echo "Всего вопросов в базе: ".mysql_result(mysql_query("SELECT COUNT(*) FROM `rebus_vopros`"),0)."<br />\n";
...
echo "<div class='foot'>\n";
echo "<a href='/adm_panel/settings_chat.php'>Настройки чата</a><br />\n";
echo "<a href='/adm_panel/chat_shut.php'>Шутки</a><br />\n";
echo "<a href='/adm_panel/chat_vopr.php'>Вопросы викторины</a><br />\n";
