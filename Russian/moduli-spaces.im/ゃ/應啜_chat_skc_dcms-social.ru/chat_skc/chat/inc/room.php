...
if($room['hud'] ==1)include 'inc/hud.php';
if ($room['shutnik'] == 1)include 'inc/shutnik.php';
if ($room['anagrama'] == 1)include 'inc/anagrama.php';
if($room['flirt'] == 1)include 'inc/flirt.php';
if ($room['rebus'] == 1)include 'inc/rebus.php';
err();
aut();
if (isset($user)){
...
{echo '<div class="nav2">';
$num=0;}
if ($post['umnik_st']==0 && $post['shutnik']==0 && $post['hud_st']==0 && $post['anagrama_st']==0 && $post['rebus_st']==0)
$ank=mysql_fetch_assoc(mysql_query("SELECT * FROM `user` WHERE `id` = $post[id_user] LIMIT 1"));
if ($post['umnik_st']==0 && $post['shutnik']==0 && $post['hud_st']==0 && $post['anagrama_st']==0 && $post['rebus_st']==0)
echo group($ank['id']);
elseif ($post['shutnik']==1)
echo "<img src='/style/themes/$set[set_them]/chat/14/shutnik.png' alt='' />\n";
elseif ($post['hud_st']!=0)echo "<img src='/style/themes/$set[set_them]/chat/14/umnik.png' alt='' />\n";
elseif ($post['anagrama_st']!=0)echo "<img src='/style/themes/$set[set_them]/chat/14/umnik.png' alt='' />\n";
elseif ($post['rebus_st']!=0)echo "<img src='/style/themes/$set[set_them]/chat/14/umnik.png' alt='' />\n";
...
elseif ($post['hud_st']==1)
echo "$set[chat_hud](".vremja($post['time']).")\n";
elseif ($post['anagrama_st']==1)echo "$set[chat_anagrama](".vremja($post['time']).")\n";
elseif ($post['rebus_st']==1)echo "$set[chat_rebus](".vremja($post['time']).")\n";
