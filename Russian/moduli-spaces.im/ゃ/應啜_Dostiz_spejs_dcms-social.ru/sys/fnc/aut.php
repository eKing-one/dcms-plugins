...
function medal_ikey($user = 0)
{
	$ank = mysql_fetch_array(mysql_query("SELECT `ikey_reit` FROM `user` WHERE `id` = $user LIMIT 1"));
	
	$img = 0;

	if ($ank['ikey_reit'] >= 10 && $ank['ikey_reit'] <= 30)
		$img = 1;
		
	elseif ($ank['ikey_reit'] >= 31 && $ank['ikey_reit'] <= 50)
		$img = 2;

	elseif ($ank['ikey_reit'] >= 51 && $ank['ikey_reit'] <= 9999999)
		$img = 3;
	...
	if ($img != 0)
	{
		return ' <img src="/reit/img/medal/' . $img . '.gif" alt="iKey" />';
	}
}
?>
