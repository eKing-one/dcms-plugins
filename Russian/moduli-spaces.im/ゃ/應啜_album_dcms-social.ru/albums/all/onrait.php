...
$set['title']='Новые фотографии'; // заголовок страницы
...
case '1':$rr='where `time` > '.($time-86400);$s=1;break;
case '2':$rr='where `time` > '.($time-604800);$s=2;break;
case '3':$rr='where `time` > '.($time-2592000);$s=3;break;
}
...
$k_post=mysql_result(mysql_query("SELECT COUNT(*) FROM `albums_foto` ".$rr." "),0);
...
$q=mysql_query("SELECT * FROM `albums_foto` ".$rr."  ORDER BY `time` DESC LIMIT $start, $set[p_str]");
...
echo "<td class='p_m' colspan='2'>Добавлена: ".vremja($post['time'])."<br />Автор: <a href='/info.php?id=$ank[id]'>$ank[nick]</a><br /></td></tr>";
