...
if (isset($_POST['save']))
{
if (isset($_POST['pod']) && strlen($_POST['pod'])>0 && strlen($_POST['pod'])<=11)
{
$sms['pod']=$_POST['pod'];

mysql_query("UPDATE `sms_set` SET `pod` = '".esc(stripcslashes(htmlspecialchars($sms['pod'])))."' WHERE `id` = '1'");
} else $err='Неверная подпись';
if (isset($_POST['login']) && (strlen($_POST['login'])>0 && strlen($_POST['login'])<=15 || $_POST['login']==NULL))
{
$sms['login']=$_POST['login'];

mysql_query("UPDATE `sms_set` SET `login` = '".esc(stripcslashes(htmlspecialchars($sms['login'])))."' WHERE `id` = '1'");
} else $err='Неверная длина логина';
if (isset($_POST['pwd']) && (strlen($_POST['pwd'])>0 && strlen($_POST['pwd'])<=35 || $_POST['pwd']==NULL))
{
$sms['pwd']=($_POST['pwd']);
mysql_query("UPDATE `sms_set` SET `pwd` = '".esc(stripcslashes(htmlspecialchars($sms['pwd'])))."' WHERE `id` = '1'");
} else $err='Неверная длина пароля';
...
echo "<div class='foot'>\n";
echo "Настройки смс оповещения<br/>\n";
echo "</div>\n";
echo "Логин:<br />\n<input type='text' name='login' value='$sms[login]' maxlength='35' /><br />\n";
echo "Пароль:<br />\n<input type='text' name='pwd' value='$sms[pwd]' maxlength='35' /><br />\n";
echo "Подпись в смс:<br />\n<input type='text' name='pod' value='$sms[pod]' maxlength='13' /><br />\n";
