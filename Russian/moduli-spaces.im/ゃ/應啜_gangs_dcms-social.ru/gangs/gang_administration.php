...
$set['title']="Высший состав ".htmlspecialchars($gang['name'])."";
...
$k_post=mysql_result(mysql_query("SELECT COUNT(*) FROM `gangs_users` WHERE `id_gang`='".$gang['id']."' AND `type`='0' AND `status`>'0'"),0);
$k_page=k_page($k_post,$set['p_str']);
$page=page($k_page);
$start=$set['p_str']*$page-$set['p_str'];
if ($k_post==0)echo "<div class='mess'>Высший состав этой банды пуст.</div>";
$q=mysql_query("SELECT * FROM `gangs_users` WHERE `id_gang`='".$gang['id']."' AND `type`='0' AND `status`>'0' ORDER BY `time` DESC LIMIT $start, $set[p_str]");
