...
if (($ank['id']==$user['id'] || user_access('foto_foto_edit')) && isset($_GET['del']))
{
$komm=mysql_fetch_assoc(mysql_query("SELECT * FROM `gallery_komm` WHERE `id` = '".intval($_GET['del'])."'  LIMIT 1"));
if ($komm['id_foto'] == $foto['id'])
mysql_query("DELETE FROM `gallery_komm` WHERE `id` = '$komm[id]' LIMIT 1");
...
if ($ank['id']!=$user['id'])mysql_query("INSERT INTO `jurnal` (`id_user`, `id_kont`, `msg`, `time`) values('0', '$ank[id]', '$user[nick] оставил [url=/foto/$ank[id]/$gallery[id]/komm/$foto[id]/]комментарий к вашему фото[/url]', '$time')");
...
if ($ank['id']==$user['id'] || user_access('foto_foto_edit'))echo "<div align=right><a href='?del=$post[id]'><img src='/style/icons/crs2.png' alt=''/></a></div>\n";
echo "  </td>\n";
echo "   </tr>\n";
