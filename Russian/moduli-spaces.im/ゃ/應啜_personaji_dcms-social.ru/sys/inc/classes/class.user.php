	...
	*/
	...
	public static function avatar($user = 0, $type = 0)
	{
		...
		*/
		...
		$ank = mysql_fetch_assoc(mysql_query('SELECT `pol`, `id`, `group_access`, `pers`, `pers_time` FROM `user` WHERE `id` = "' . $user . '" LIMIT 1 '));
		...
		if ($type == 0 || $type == 1)
		{
			if ($ank['pers_time'] > time() - 60 * 60 * 24 * 7)
			{
				...
				?>
				<div style="text-align: center; width: 50px; height:50px; display: inline-block; z-index: 5; position: relative; overflow: hidden; vertical-align: middle;">
				<img src="/style/person/head_small/h (<?=$head['id']?>).png"  style="z-index: 1; position: relative;" />
				<br />
				<img src="/style/person/foot_small/f (<?=$foot['id']?>).png"  style="<?=$head['style_small']?> position: relative;" />
				<br />
				</div>
				<?
			}
			...
			{
				$avatar = mysql_fetch_array(mysql_query("SELECT id,id_gallery,ras FROM `gallery_foto` WHERE `id_user` = '$user' AND `avatar` = '1' LIMIT 1"));
				
				if (is_file(H."sys/gallery/50/$avatar[id].$avatar[ras]"))
				{
					return '<img class="avatar" src="/foto/foto50/' . $avatar['id'] . '.' . $avatar['ras'] . '" alt="Avatar"  width="50" />';
				}
				else
				{
