...
<?  
  ...
  $Out = '<a href="/FotoKonkurs/Konkurs.php?ID='.$Konkurs['id'].'">'.text_out($Konkurs['name']).'</a> '.$Raquo.' <a href="/FotoKonkurs/Photo.php?ID='.$ID.'">'.text_out($Photo['name']).'</a> '.$Raquo.' <a href="/FotoKonkurs/DeletePhoto.php?ID='.$ID.'">Удалить</a>';
  ...
  
  {
    ...
    if (isset($_POST['submit_yes'])) {
	  ...
	  echo $div_link.'Фотография успешно удалена из конкурса.<br/></div>';
	  
	  $ListComments = mysql_query("select * from `FotoKonkursComments` WHERE `photo_id` = '".$ID."'");
	  $ListRating = mysql_query("select * from `FotoKonkursRating` WHERE `photo_id` = '".$ID."'");
	  
	  while ($List = mysql_fetch_array($ListComments)) { mysql_query("DELETE FROM `FotoKonkursComments` WHERE `id` = '".$List['id']."'"); }
	  while ($List2 = mysql_fetch_array($ListRating)) { mysql_query("DELETE FROM `FotoKonkursRating` WHERE `id` = '".$List2['id']."'"); }
	  
	  unlink('Files/Wiev/'.$Photo['id'].'_'.$Photo['foto']);
	  unlink('Files/Original/'.$Photo['id'].'_'.$Photo['foto']);
	  
	  mysql_query("DELETE FROM `FotoKonkursUser` WHERE `id` = '".$ID."'");
	
	}else{

	   echo $div_link;
       echo '<font color="red">Вы действительно хотите удалить фотографию из конкурса?</font><br/>
	         <form method="post">
	         <input type="submit" name="submit_yes" value="Да, я хочу удалить фото"/></form>
			 <form action="/FotoKonkurs/Photo.php?ID='.$ID.'" method="post">
